 ...
 */
/* ----------------------------------------------------------------------
 * Title:        adi_sharcfx_depthwise_conv_s8.c
        ...
        for (int32_t in_w = -pad_x, out_w = 0, ker_h_start = MAX(0, -in_h); out_w < output_x; in_w += stride_x, ++out_w)
        {
                 ...
                 ++in_ch, out_ch += ch_mult)
            {
                for (int mult_tile = 0; mult_tile < ch_mult; mult_tile += 4)
                {
                    ...
                    out_buff[0] = adi_sharcfx_nn_requantize(out_buff[0], *output_mult++, *output_shift++);
                    out_buff[1] = adi_sharcfx_nn_requantize(out_buff[1], *output_mult++, *output_shift++);
                    out_buff[2] = adi_sharcfx_nn_requantize(out_buff[2], *output_mult++, *output_shift++);
                    out_buff[3] = adi_sharcfx_nn_requantize(out_buff[3], *output_mult++, *output_shift++);
            ...
            for (int i_out_x = 0; i_out_x < output_x; i_out_x++)
            {
                ...
                for (int i_input_ch = 0; i_input_ch < input_ch; i_input_ch++)
                {
                    for (int i_ch_mult = 0; i_ch_mult < ch_mult; i_ch_mult++)
                    {
                        ...
                        acc_0 = adi_sharcfx_nn_requantize(acc_0, output_mult[idx_out_ch], output_shift[idx_out_ch]);
                        acc_0 += output_offset;
