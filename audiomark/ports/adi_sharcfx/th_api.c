...
th_cfft_init_f32(ee_cfft_f32_t *p_instance, int fft_length)
{
    ...
    status = adi_sharcfx_cfft_init_f32(p_instance, fft_length);
    if (!status)
    {
...
{
    adi_sharcfx_cfft_f32(p_instance, p_buf, ifftFlag, bitReverseFlagR);
}
...
th_rfft_init_f32(ee_rfft_f32_t *p_instance, int fft_length)
{
    ...
    status = adi_sharcfx_rfft_fast_init_f32(p_instance, fft_length);
    if (!status)
    {
        return EE_STATUS_ERROR;
    }
...
th_rfft_f32(ee_rfft_f32_t *p_instance,
            ...
            uint8_t        ifftFlag)
{
    adi_sharcfx_rfft_fast_f32(p_instance, p_in, p_out, ifftFlag);
}
...
th_absmax_f32(const ee_f32_t *p_in,
              ...
              uint32_t       *p_index)
{
    adi_sharcfx_absmax_f32(p_in, len, p_max, p_index);
}
...
th_cmplx_mult_cmplx_f32(const ee_f32_t *p_a,
                        ...
                        uint32_t        len)
{
    adi_sharcfx_cmplx_mult_cmplx_f32(p_a, p_b, p_c, len);
}

void
th_cmplx_conj_f32(const ee_f32_t *p_a, ee_f32_t *p_c, uint32_t len)
{
    adi_sharcfx_cmplx_conj_f32(p_a, p_c, len);
}
...
ds_cnn_s_s8_get_buffer_size(void)
{
    ...
    int32_t size = adi_sharcfx_convolve_wrapper_s8_get_buffer_size(
    ...
    size = adi_sharcfx_depthwise_conv_wrapper_s8_get_buffer_size(
 ...
 * https://github.com/ADI_SHARCFX-software/ML-zoo & Tag: 22.02 */
nn_context ctx;

void
th_nn_init(void)
{
    // unused const adi_sharcfx_nn_status expected = ADI_SHARCFX_NN_SUCCESS;
...
th_nn_classify(const input_tensor_t in_data, output_tensor_t out_data)
{
    ...
    nn_dims                     conv_filter_dims;
    nn_dims                     dw_conv_filter_dims;
    nn_dims                     in_out_dim_1;
    nn_dims                     bias_dims;
    ...
    adi_sharcfx_nn_status status
        = adi_sharcfx_convolve_wrapper_s8(&ctx,
                                  &conv_params,
                                  &quant_params,
                                  &in_out_dim_0,
                                  in_data,
    ...
    status |= adi_sharcfx_depthwise_conv_wrapper_s8(&ctx,
                                            &dw_conv_params,
                                            &quant_params,
                                            &in_out_dim_1,
                                            in_out_buf_0,
    ...
    status |= adi_sharcfx_convolve_wrapper_s8(&ctx,
                                      &conv_params,
                                      &quant_params,
                                      &in_out_dim_0,
                                      in_out_buf_1,
    ...
    status |= adi_sharcfx_avgpool_s8(&ctx,
                             &pool_params,
                             &in_out_dim_1,
                             in_out_buf_0,
                             &conv_filter_dims,
    ...
    nn_per_tensor_quant_params per_tensor_quant_params;
    ...
    status |= adi_sharcfx_fully_connected_s8(&ctx,
                                     &fc_params,
    ...
    adi_sharcfx_softmax_s8(in_out_buf_0,
                   SOFTMAX_12_NUM_ROWS,
                   SOFTMAX_12_ROW_SIZE,
                   SOFTMAX_12_MULT,
                   SOFTMAX_12_SHIFT,
    ...
    return status == ADI_SHARCFX_NN_SUCCESS ? EE_STATUS_OK : EE_STATUS_ERROR;
}
